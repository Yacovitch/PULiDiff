...
@click.command()
@click.option('--diff', '-d', type=str, default='/nas2/jacob/LiDiff/lidiff/checkpoints/diff_net.ckpt', help='path to the scan sequence')
...
def main(diff, refine, denoising_steps, cond_weight):
    ...
    for pcd_path in tqdm.tqdm(natsorted(os.listdir(path))):
        ...
        start = time.time()
        refine_scan, diff_scan, scan = diff_completion.complete_scan(points)
        print(diff_scan.shape)
        end = time.time()
        ...
        o3d.io.write_point_cloud(f'/nas2/jacob/LiDiff/lidiff/results/{exp_dir}/refine/{pcd_path.split(".")[0]}.ply', pcd_refine)
        print(f'./results/{exp_dir}/refine/{pcd_path.split(".")[0]}.ply')
        ...
        o3d.io.write_point_cloud(f'/nas2/jacob/LiDiff/lidiff/results/{exp_dir}/diff/{pcd_path.split(".")[0]}.ply', pcd_scan)
