...
def point_set_to_sparse_sensat(p_full, p_part, n_full, n_part, resolution, filename, p_mean=None, p_std=None):
    concat_part = np.ceil(n_part / p_part.shape[0])
    p_part = p_part[torch.randperm(p_part.shape[0])]
    p_part = p_part.repeat(concat_part, 0)
    #n = p_part.shape[0]
...
def point_set_to_sparse_grid(p_full, p_part, n_full, n_part, resolution, filename, p_mean=None, p_std=None):
    ...
    #pcd_part.points = o3d.utility.Vector3dVector(p_part)
    #viewpoint_grid = o3d.geometry.VoxelGrid.create_from_point_cloud(pcd_part, voxel_size=10.)
    #pcd_part = pcd_part.voxel_down_sample(voxel_size=0.2)
    #pcd_part = np.array(pcd_part.points)
