...
class MinkowskiEncoder256(nn.Module):
    def __init__(self, in_channels=512, feature_dim=256, D=3):
        """
        A 5-layer Minkowski sparse convolutional encoder that takes 512D input features
        and maintains a 256D feature dimension throughout the network.

        Args:
            in_channels (int): The number of input feature dimensions (default: 512).
            feature_dim (int): The number of output feature dimensions per layer (default: 256).
            D (int): The number of spatial dimensions (3 for 3D point clouds).
        """
        super().__init__()

        self.D = D  # Spatial dimension (3D)
        
        # Initial Convolution Layer (Stem)
        self.stem = nn.Sequential(
            ME.MinkowskiConvolution(in_channels, feature_dim, kernel_size=3, stride=1, dimension=self.D),
            ME.MinkowskiBatchNorm(feature_dim),
            ME.MinkowskiReLU(inplace=True),
            ME.MinkowskiConvolution(feature_dim, feature_dim, kernel_size=3, stride=1, dimension=self.D),
            ME.MinkowskiBatchNorm(feature_dim),
            ME.MinkowskiReLU(inplace=True)
        )

        # Stage 1: Downsampling & Processing (Stride 2)
        self.stage1 = nn.Sequential(
            ME.MinkowskiConvolution(feature_dim, feature_dim, kernel_size=2, stride=2, dimension=self.D),
            ME.MinkowskiBatchNorm(feature_dim),
            ME.MinkowskiReLU(inplace=True),
            ME.MinkowskiConvolution(feature_dim, feature_dim, kernel_size=3, stride=1, dimension=self.D),
            ME.MinkowskiBatchNorm(feature_dim),
        ...
        # Stage 2: Downsampling & Processing (Stride 2)
        self.stage2 = nn.Sequential(
            ME.MinkowskiConvolution(feature_dim, feature_dim, kernel_size=2, stride=2, dimension=self.D),
            ME.MinkowskiBatchNorm(feature_dim),
            ME.MinkowskiReLU(inplace=True),
        ...
        # Stage 3: Downsampling & Processing (Stride 2)
        self.stage3 = nn.Sequential(
            ME.MinkowskiConvolution(feature_dim, feature_dim, kernel_size=2, stride=2, dimension=self.D),
            ME.MinkowskiBatchNorm(feature_dim),
            ME.MinkowskiReLU(inplace=True),
        ...
        # Stage 4: Downsampling & Processing (Stride 2)
        self.stage4 = nn.Sequential(
            ME.MinkowskiConvolution(feature_dim, feature_dim, kernel_size=2, stride=2, dimension=self.D),
            ME.MinkowskiBatchNorm(feature_dim),
            ME.MinkowskiReLU(inplace=True),
    ...
    def forward(self, x):
        """
        Forward pass through the 5-layer Minkowski encoder.

        Args:
            x (ME.TensorField or ME.SparseTensor): Input sparse tensor with 512D features.

        Returns:
            ME.SparseTensor: Encoded sparse tensor with spatially downsampled representation.
        """
        x0 = self.stem(x.sparse())  # Convert to SparseTensor
        x1 = self.stage1(x0)
        x2 = self.stage2(x1)
        x3 = self.stage3(x2)
        x4 = self.stage4(x3)  # Final stage

        return x4  # Output remains at 256D per voxel
...
class MinkUNetDiffClip(nn.Module):
    def __init__(self, **kwargs):
        super().__init__()

        cr = kwargs.get('cr', 1.0)
        in_channels = kwargs.get('in_channels', 3)
        cs = [32, 32, 64, 128, 512, 512, 128, 96, 96]
        cs = [int(cr * x) for x in cs] 
        self.embed_dim = cs[-1]
        self.run_up = kwargs.get('run_up', True)
        self.D = kwargs.get('D', 3)
