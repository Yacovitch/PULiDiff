...
def points2grid(points, params):
    ...
    z_int = _z.ceil().to(device)  # (batch_size * view_num) * 1024 * 1

    _x = torch.clip(_x, 1, params['resolution'] - 2).to(device)
    _y = torch.clip(_y, 1, params['resolution'] - 2).to(device)
    _z = torch.clip(_z, 1, depth - 2).to(device)
    ...
    grid = grid.index_put(index, _z.view(-1, )).permute((0, 1, 3, 2)).to(device)

    return grid.squeeze(), _x, _y, z_int, _z, nbatch


class RealisticProjection(nn.Module):
    """For creating images from PC based on the view information.
    """

    def __init__(self, params, device, n_points=1024):
        super().__init__()
        _views = np.asarray([
    ...
    def get_img(self, points, is_cls=False):
        ...
        grid, xx, yy, zz_int, zz, nnbatch = points2grid(points=self._points, params=self.params)
        if is_cls:
    ...
    def point_transform(points, rot_mat, rot_mat2, rot_mat3, translation):
        ...
        #print('rot_mat3: ', rot_mat3.shape, rot_mat3.device)
        rot_mat = rot_mat.to(points.device)
        rot_mat2 = rot_mat2.to(points.device)
        rot_mat3 = rot_mat3.to(points.device)
        translation = translation.to(points.device)
        points = torch.matmul(points.float(), rot_mat)
        points = torch.matmul(points.float(), rot_mat2)
        points = torch.matmul(points.float(), rot_mat3)
